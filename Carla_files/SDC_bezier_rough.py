...
def find_trajectory(mask, world_x_left, world_z_left, world_x_right, world_z_right, min_points):
    ...
    if direction == [1,0,0,0]:
        ...
        # Refer to your diagram. There are 5 main points. P, Q, R, S, intersection_point
        # P is your vehicle location
        #  Q and R are intermediate points
        # S is the final point
        ...
        point_S = [None,None]
        # point_S[0] = (final_slope*point_R[0] - point_R[1] - intersect_line_slope*world_x_left[-1] + world_z_left[-1])/(-intersect_line_slope + final_slope)
        # point_S[1] = final_slope*point_S[0] + point_R[1] - point_R[0]*final_slope 
        # point_S = [farthest_point_x ,point_R[1] - final_slope*point_R[0] + final_slope*farthest_point_x]
...
def bezier_split(trajectory_points, num_points):
    ...
    new_trajectory.append(list(start_point))

    buff_dist =0
    for i in range(len(trajectory_points)-1):
        a = trajectory_points[i+1][0] - start_point[0]
        b = trajectory_points[i+1][1] - start_point[1] 
        dir_vec = [a/np.sqrt(a**2+b**2), b/np.sqrt(a**2+b**2)]
        rem_dist = np.linalg.norm(np.array(start_point) - np.array(trajectory_points[i+1]))
        check =0
        append_check = 0
        prev_rem_dist = rem_dist + 0.000000000001
        while check == 0 and rem_dist < prev_rem_dist:
            prev_rem_dist = rem_dist
            if rem_dist - (seg_len - buff_dist) >= 0 :
                new_trajectory.append([round(start_point[0] + dir_vec[0]*(seg_len - buff_dist),3), 
                                       round(start_point[1] + dir_vec[1]*(seg_len - buff_dist),3)])
                rem_dist = rem_dist - (seg_len - buff_dist)
                start_point = new_trajectory[-1]
                buff_dist = 0
                append_check = 1
            else:

                buff_dist = rem_dist*append_check
                check = 1
...
def bezier_plot(left_world_coord,right_world_coord,mask_image,points):
    ...
    return mask2, open_cv_trajectory, trajectory, centre_x,centre_y
